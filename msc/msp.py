...
class MSPStatus(Enum):
    ...
    MSP_ERROR_OVERFLOW = (
        10113,
        """ 0x2781 """ """ 单用户下模型数超上限(10个),只出现在测试时对一个用户进行并发注册 """,
    )
    ...
    MSP_ERROR_NET_DNS = (
        10214,
        """ 0x27E6 """
        """ domain name is invalid or dns server does not function well """,
    )
    ...
    MSP_ERROR_MMP_STORE_MNR_NO_INIT = (
        15012,
...
def MSPDownload(
    ...
    resultCb: _FuncPointer,
    userData: bytes,
):
    dataName = dataName.encode("UTF-8") if dataName else None
    params = params.encode("UTF-8") if params else None
    errorCode: int = msc.MSPDownload(dataName, params, statusCb, resultCb, userData)
